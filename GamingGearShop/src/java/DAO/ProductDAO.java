...
public class ProductDAO extends JPAGenericDAO<ProductDTO> {
    ...
    public List<ProductDTO> getAllProducts() {
        String jpql = "SELECT p FROM ProductDTO p "
                + "JOIN CategoryDTO c ON p.catID = c.catID "
                + "JOIN BrandDTO b ON p.brandID = b.brandID "
                + "WHERE p.status = true AND c.status = true AND b.status = true";
    ...
    public int getCountProduct() {
        ...
        try {
            ...
            String jpql = "SELECT COUNT(p) FROM ProductDTO p "
                    + "JOIN CategoryDTO c ON p.catID = c.catID "
                    + "JOIN BrandDTO b ON p.brandID = b.brandID "
                    + "WHERE p.status = true AND c.status = true AND b.status = true";
